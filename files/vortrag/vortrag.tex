...
\title[EvoAlg]{Evolutionï¿½re Algorithmen zur Minimierung der Griewank-Funktion}   
...
\begin{frame}\frametitle{Gliederung}
	...
	\item \textbf{\large Einleitung}\\
		Aufbau eines evolutionï¿½ren Algorithmus, Strategie- und Parameterwahl, Auswertung \\[0.4cm]
	\item \textbf{\large Ergebnisse fï¿½r konstante Mutationsrate}\\[0.4cm]
	\item \textbf{\large Ergebnisse fï¿½r mit Alter wachsender Mutationsrate}\\[0.4cm]
	\item \textbf{\large Ergebnisse fï¿½r mit Alter abnehmender Mutationsrate}\\
    \end{enumerate}
\end{frame}
		...
		\begin{equation*}
			...
			\begin{array}{cl}
				(1) & \text{Initialisierung der Anfangs-}\\
					& \text{population zufï¿½llig} \\[0.2cm]
				(2) & \text{Umwelt- und Elternselektion }\\
					& \text{in Teilen zufallsabhï¿½ngig} \\[0.2cm]
				(3) & \text{Mutation zufallsabhï¿½ngig} \\
			\end{array}
...
\begin{frame}\frametitle{Optimale Fitnesswerte fï¿½r $n = 2$} 

\end{frame}

\begin{frame}\frametitle{Mittlere Generationenanzahl bis Abbruch fï¿½r $n = 2$} 
...
\section{Ergebnisse altersabhï¿½ngige Mutationsrate}

\begin{frame}
    
    \begin{center}
	\LARGE{Altersabhï¿½ngige Mutationsrate}
    \end{center}
...
\begin{frame}\frametitle{Mittleres Minimum fï¿½r $n = 2$} 
